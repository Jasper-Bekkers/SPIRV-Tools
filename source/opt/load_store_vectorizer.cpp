...
bool LoadStoreVectorizerPass::RunOnFunction(ir::Function* func){ 
	...
	for (auto bi = func->begin(); bi != func->end();) {
		...
		if (c)
		{
			std::unique_ptr<ir::BasicBlock> newBB(new ir::BasicBlock(std::move(NewLabel(bi->id()))));

			for (auto& i : basicBlockInstructions)
			{
				newBB->AddInstruction(std::unique_ptr<ir::Instruction>(new ir::Instruction( i)));
			}
...
bool LoadStoreVectorizerPass::VectorizeChains(InstVec* block_ptr, InstrListMap& map)
{
	bool changed = false;
	...
	for (auto& chain : map)
	{
		...
		for (unsigned CI = 0, CE = size; CI < CE; CI += 64) 
		{
			...
			std::vector<ir::Instruction *> chunk(chain.second.begin() + CI, chain.second.begin() + CI + len);
...
bool LoadStoreVectorizerPass::isConsecutiveAccess(ir::Instruction *a, ir::Instruction *b)
{
	...
	if (ptrA->opcode() == SpvOpAccessChain && ptrB->opcode() == SpvOpAccessChain)
	{
		// check if 
		// - type matches
		// - variable matches
		...
		// do this by just checking all the dwords except for the last one (this gets rid of the first 3 checks)

		if (ptrA->NumOperands() != ptrB->NumOperands())
			return false;
		...
		for (uint32_t opIdx = 0; opIdx < numOps - 1; opIdx++)
		{
			...
			if (opA.type != opA.type)
				sameStart = false;
			// don't check the ResultId since it will never match
			if (opA.type == SPV_OPERAND_TYPE_RESULT_ID)
				continue;
			if (opB.type == SPV_OPERAND_TYPE_RESULT_ID)
				continue;
		...
		if (aInst->opcode() == SpvOpConstant && bInst->opcode() == SpvOpConstant)
		{
			...
			// SPIR-V doesn't store byte offsets but element offsets so we can just compare with 1 here
...
bool LoadStoreVectorizerPass::VectorizeInstructions(InstVec* block_ptr, std::vector<ir::Instruction *>& instrs)
{
	...
	for (int Head : Heads) {
		...
		//else
		Vectorized = vectorizeStoreChain(block_ptr, Operands, &InstructionsProcessed);
...
bool LoadStoreVectorizerPass::vectorizeStoreChain(InstVec* block_ptr, std::vector<ir::Instruction *> operands, std::set<ir::Instruction *>* processed)
{
	// 0. Find or create an OpTypeVector
	// 1. Create an OpCompositeConstruct with all the same arguments as the OpStores that are in the operands list
	// 2. Replace all OpStore and OpAccessChain's leading here, with a single OpStore and OpAccessChain
	...
	if (opTypeVector)
	{
		...
		uint32_t replId = TakeNextId();
		{
			...
			ir::Instruction newComposite(
				SpvOpConstantComposite, 0, opTypeVector->result_id(),
				ops);
		...
		uint32_t opTypePointerId = TakeNextId();
		{
			ir::Instruction type_inst(
				...
				{ spv_operand_type_t::SPV_OPERAND_TYPE_ID,{ opTypeVector->result_id() } } });
		...
		uint32_t accessChainStoreId = TakeNextId();
		{
			...
			ir::Instruction newAccessStore(
				SpvOpAccessChain, 0, opTypePointerId, newACOps
			);
		...
		// just nop out the now redundant stores
		for (auto& oper : operands)
		{
			auto foundIt = FindInBasicBlock(block_ptr, *oper);
...
ir::Instruction* LoadStoreVectorizerPass::findVectorInOpAccessChain(ir::Instruction* opLoadOrOpStore)
{
	uint32_t dummy;
	...
	auto baseIdIndex = 2;
	auto baseInstr = def_use_mgr_->GetDef(opAccessChain->GetSingleWordOperand(baseIdIndex));
	auto baseTypeInstr = def_use_mgr_->GetDef(baseInstr->GetSingleWordOperand(0));
	
	auto typePointedTo = def_use_mgr_->GetDef(baseTypeInstr->GetSingleWordOperand(2));
	for (size_t i = 3; i < opAccessChain->NumOperands(); ++i) {
		...
		case SpvOpTypeRuntimeArray: {
			...
			typePointedTo = def_use_mgr_->GetDef(typePointedTo->GetSingleWordOperand(1));
			break;
		}
		case SpvOpTypeStruct: {
			const uint32_t cur_index = cur_word_instr->GetSingleWordOperand(2);
			auto structMemberId = typePointedTo->GetSingleWordOperand(cur_index + 1);
...
inline bool LoadStoreVectorizerPass::IsNonPtrAccessChain(const SpvOp opcode) const {
	return opcode == SpvOpAccessChain || opcode == SpvOpInBoundsAccessChain;
}

inline ir::Instruction * LoadStoreVectorizerPass::GetPtr(uint32_t ptrId, uint32_t * varId) {
	*varId = ptrId;
...
inline ir::Instruction * LoadStoreVectorizerPass::GetPtr(ir::Instruction * ip, uint32_t * varId) {
